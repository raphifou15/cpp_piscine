...
void    Bureaucrat::signForm( Form &i)
{
    try
    {
        if (this->getGrade() > i.getGradeSign())
            throw   Bureaucrat::GradeTooLowException();
        i.beSigned(*this);
        std::cout << this->getName() << " signe " << i.getName() << std::endl;
    }
    catch(const std::exception& e)
    {
        std::cerr << this->getName() << " cannot sign the document " << i.getName() <<" because " << this->getName() << " " << e.what() << std::endl;
    }
}
...
void    Bureaucrat::executeForm (Form const & form)
{
    ...
    {
        std::cerr << this->getName() << " cannot exec the document " << form.getName() <<" because " << this->getName() << " " << e.what() << std::endl;
    }
}
